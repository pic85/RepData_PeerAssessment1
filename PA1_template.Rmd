...
The mean of steps taken per day is `r media` and the median is `r mediana`.
...
The 5-minute interval number `r max_interval` contains on average the maximum number of steps.

```{r}
max_steps <- round(max(intervals$mean_steps),0)
```

The average number of steps for this 5-minute interval is `r max_steps `.
...
The mean steps per day with the imputed data is `r media_imp` and median is `r mediana_imp`.
